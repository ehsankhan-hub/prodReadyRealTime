...
class RemoteLogCurveDataSource extends LogCurveDataSource {
  ...
  public parseAndAppendData(logData: LogData): void {
    ...
    const timeIdx = mnemonics.indexOf('TIME') !== -1 ? mnemonics.indexOf('TIME') : mnemonics.indexOf('DATE');
    const curveIdx = mnemonics.indexOf(this.mnemonic);

    if (timeIdx === -1 || curveIdx === -1) {
      console.warn(`⚠️ Mnemonic not found. TIME: ${timeIdx}, ${this.mnemonic}: ${curveIdx}. Available: ${mnemonics.join(',')}`);
      return;
    }

    const viewport = (this as any)._parentComponent.wellLogWidget?.getVisibleDepthLimits();
    const currentViewMid = viewport ? (viewport.getLow() + viewport.getHigh()) / 2 : null;
    ...
    const newData = logData.data
      .map(row => {
        ...
        const d = (timeVal.includes('T') || timeVal.includes('-')) ? new Date(timeVal).getTime() : parseFloat(timeVal);
        const v = this.isMudLog ? (cols[curveIdx]?.trim() || 'UNKNOWN') : parseFloat(cols[curveIdx]);
        return isNaN(d) ? null : { d, v };
      })
      .filter((entry): entry is { d: number, v: any } => entry !== null);
...
@Component({
  selector: 'app-generate-canvas-time-tracks',
  standalone: true,
  imports: [
    CommonModule,
    FormsModule,
...
export class GenerateCanvasTimeTracksComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  private loadLogHeadersAndCreateTracks(): void {
    ...
    this.logHeadersService.getTimeLogHeaders(this.well, this.wellbore).subscribe({
      next: (headers) => {
  ...
  private processLogHeaders(headers: LogHeader[]): void {
    ...
    headers.forEach((h) => {
      const startStr = h.startIndex?.['#text'];
      const endStr = h.endIndex?.['#text'];
      if (startStr && endStr) {
    ...
    if (this.headerMinDepth === Number.MAX_SAFE_INTEGER) this.headerMinDepth = 0;
  ...
  private loadMudLogData(curve: TrackCurve): Observable<any[]> {
    ...
    return this.http.get<Array<{ depth: number, value: string }>>('/assets/data/mudLogTimeData.json').pipe(
      map((mudLogData) => {
  ...
  private async createSceneWithData(): Promise<void> {
    ...
    try {
      ...
      if (axis) {
        ...
        tickGenerator.setFormatLabelHandler((_gen: any, _parent: any, _orient: any, _tickInfo: any, _tickIndex: number, value: number) => {
      ...
      try {
        ...
        const maxTime = this.headerMaxDepth > 0 ? this.headerMaxDepth : this.getMaxDepth();
        ...
        const viewScale = this.selectedScale > 0 ? this.selectedScale : defaultViewRange;
        ...
        this.wellLogWidget.on('VisibleDepthLimitsChanged', (_evt: any, _sender: any, args: any) => {
          const newLimits = args?.new;
          if (newLimits) {
            console.log(`📜 [Scroll] Visible range changed: ${new Date(newLimits.getLow()).toISOString()} to ${new Date(newLimits.getHigh()).toISOString()}`);
          }
          this.ngZone.run(() => {
  ...
  private create2DVisual(
    ...
    const colors = new DefaultColorProvider()
      .addColor(0.00, 'rgba(124, 179, 66, 1)')  // Green
