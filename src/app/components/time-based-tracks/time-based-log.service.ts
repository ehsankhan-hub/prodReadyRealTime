...
export class TimeBasedLogService {
  ...
  /**
   * Converts time-based data to GeoToolkit format
   */
  convertTimeDataToGeoToolkitFormat(logData: any): any {
    try {
      console.log('🔄 Converting time-based data to GeoToolkit format...');
      
      const convertedData = {
        logs: logData.logs.map((log: any) => ({
          logId: log.logId,
          endDateTimeIndex: log.endDateTimeIndex,
          logData: {
            data: log.data.map((row: any) => {
              if (typeof row === 'string') {
                return row.split(',');
              }
              return row;
            }),
            mnemonicList: log.data[0] ? Object.keys(log.data[0]).join(',') : '',
            unitList: this.extractUnitsFromData(log.data)
          }
        }))
      };

      console.log('✅ Time-based data converted successfully');
      return convertedData;

    } catch (error) {
      console.error('❌ Error converting time-based data:', error);
      throw error;
    }
  }

  /**
   * Extracts units from time-based data
   */
  private extractUnitsFromData(data: any[]): string {
  ...
  /**
   * Validates time-based query parameters
   */
  validateTimeQueryParameters(params: ILogDataQueryParameter): boolean {
    if (!params.wellUid || !params.logUid || !params.wellboreUid) {
      console.error('❌ Missing required well information');
      return false;
    }

    if (!params.startIndex || !params.endIndex) {
      console.error('❌ Missing time range');
      return false;
    }

    if (params.indexType !== 'time') {
      console.error('❌ Invalid index type for time-based data');
      return false;
    }
  ...
  /**
   * Parses time from API response
   */
  parseTimeFromAPI(timeString: string): Date {
    return new Date(timeString);
  }

  /**
   * Gets time-based chunk boundaries for data loading
   */
  getTimeChunkBoundaries(startTime: Date, endTime: Date, chunkHours: number = 1): { start: Date, end: Date }[] {
    ...
    while (currentStart < endTime) {
      const currentEnd = new Date(Math.min(currentStart.getTime() + chunkMs, endTime.getTime()));
      chunks.push({
