...
@Component({
  ...
  styleUrls: ['./time-based-tracks.component.scss'],
})
  ...
  implements OnInit, OnDestroy, AfterViewInit
{
  ...
  private loadingTimeout: any = null; // Timeout to force reset isLoading flag
  ...
  private loadData(): void {
    ...
    this.listOfTracks.forEach((trackInfo) => {
      trackInfo.curves.forEach((curve: ITimeCurve) => {
        ...
        if (!curve.LogId) {
          console.warn(
            `⚠️ Skipping curve with undefined LogId: ${
  ...
  private configureWidgetLimits(): void {
    ...
    console.log(
      `🔍 Processing ${this.wellboreObjects.length} wellbore objects`
    );
    console.log(`🔍 Matched headers:`, Array.from(this.matchedHeaders || []));
    console.log(
      `🔍 All wellbore object IDs:`,
      this.wellboreObjects.map((h) => h.objectId)
    );
    ...
    if (headerMinTime !== Infinity && headerMaxTime !== 0) {
      ...
      console.warn(
        '⚠️ No headers matched, using current data range as fallback'
      );
    ...
    console.log(
      `📊 Showing 4-hour window with full range available for scrolling`
    );
  ...
  /**
   * Safely reset loading state with timeout protection
   */
  ...
  private setLoadingState(): void {
    ...
    // Set timeout to force reset if stuck (30 seconds)
    this.loadingTimeout = setTimeout(() => {
      console.warn('⚠️ Loading timeout - forcing reset of isLoading flag');
  ...
  private checkScrollAndLoadData(): void {
    ...
    try {
      ...
      // Additional check: prevent duplicate calls with same ranges (more strict check)
      const rangeChanged =
        Math.abs(currentVisibleMin - this.lastVisibleMin) > 5000 ||
        Math.abs(currentVisibleMax - this.lastVisibleMax) > 5000; // 5 seconds minimum change

      // Only proceed if we actually need to load data and the range significantly changed
      if ((needsLoadEarlier || needsLoadLater) && rangeChanged) {
        ...
        this.scrollDebounceHandle = setTimeout(() => {
          ...
          // Use the new safe loading state method
          ...
          try {
            ...
            // Update last visible range after successful processing
            this.lastVisibleMin = currentVisibleMin;
            this.lastVisibleMax = currentVisibleMax;
          } catch (error) {
            console.error('❌ Error during scroll data loading:', error);
  ...
  private loadAdditionalLogData(
    ...
    if (!startTime || !endTime) {
      ...
      // Reset loading state if we can't proceed
      this.resetLoadingState();
      return;
    }
    ...
    if (!headerStartTime || !headerEndTime) {
      console.error('❌ Invalid date values in loadAdditionalLogData');
      // Reset loading state if we can't proceed
      this.resetLoadingState();
      return;
    }
    ...
    if (loadMin >= currentDataMin && loadMax <= currentDataMax) {
      ...
      // Reset loading state since we don't need to load anything
      this.resetLoadingState();
      return;
    }
    ...
    if (this.loadingRanges.has(rangeKey)) {
      ...
      // Reset loading state since this range is already being processed
      this.resetLoadingState();
      return;
    }
  ...
  private processAdditionalDataResponse(response: any): void {
    ...
    try {
      ...
      console.error('❌ Error processing additional data response:', error);
      // Reset loading state on any error
      this.resetLoadingState();
    }
  }
