...
const INITIAL_VIEW_RANGE_MS = 1 * 60 * 60 * 1000; // Initial load: 1 hour
...
class RemoteLogCurveDataSource extends LogCurveDataSource {
    ...
    override requestData(
        ...
        // 3. Fetch data from service - Using ISO strings
    ...
    //   const isPreParsed = !!reaponse.parsedRows;
    ...
    //       // If pre-parsed, timeVal is already a ms timestamp. Otherwise, parse it once.
    //       const d = isPreParsed
    //         ? timeVal
    ...
    //       // If pre-parsed, values are already floats/strings.
    ...
    //       // ROBUST PRUNING: Always keep the newly arrived data (newData)
    //       // to prevent race conditions during large jumps.
    ...
    //         // Never prune data that just arrived
    ...
    //         console.log(
    //           `✂️ [RemoteDS] Virtualization: Pruned ${
    //             unique.length - pruned.length
...
export class DynamicTrackTimeGeneratorComponent
    ...
    ngOnInit(): void {
        ...
        // Setup debounced listener for visible limits
        // Wait 300ms after scrolling stops before triggering multiple requestData calls
        this.visibleLimits$.pipe(debounceTime(3000)).subscribe(() => {
            ...
            this.ngZone.run(() => {
                ...
                console.log('📜 [Debounced] Triggering requestData for all curves');
    ...
    //   // Determine overall time range from headers
    //   headers.forEach((h) => {
    //     console.log('headers date ',h)
    ...
    private async processLogHeaders(headers: IWellboreObject[]): Promise<void> {
        ...
        console.log(
            `📊 Processing ${relevantHeaders.length}/${headers.length} relevant log headers for time limits`
        );
            ...
            try {
                ...
                this.wellLogWidget.on(
                    ...
                    (_evt: any, _sender: any, args: any) => {
                        // Signal the debounced Subject instead of calling logic immediately
                        this.visibleLimits$.next();
                    }
                );
    ...
    private createCurves(track: LogTrack, trackInfo: ITracks): void {
        trackInfo.curves.forEach((curveInfo, curveIndex) => {
            try {
                ...
                if (!header) {
                    console.warn(
                        'Header not found for this LogId ',
