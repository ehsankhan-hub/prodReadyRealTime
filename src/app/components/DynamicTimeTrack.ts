 ...
 */
const MAX_RETENTION_WINDOW_MS = 24 * 60 * 60 * 1000; // Keep 24 hours of history in memory
const INITIAL_VIEW_RANGE_MS = 4 * 60 * 60 * 1000; // Initial load: 4 hours
...
class RemoteLogCurveDataSource extends LogCurveDataSource {
    ...
    constructor(
        private service: WellDataService,
        private well: string,
    ...
    override requestData(
        ...
        console.log(
            `📡 [RemoteDS] Requesting ${this.mnemonic} for range: ${start}-${end} (scale: ${scale})`
        );

        // 3. Fetch data from service - Using ISO strings for clarity as requested
        const startISO = new Date(start).toISOString();
        const endISO = new Date(end).toISOString();
        // const queqyParam={well:this.well, wellBore:this.wellbore, logId:this.logId, startIndex:startISO, endIndex:endISO}
        const queryParameter: ILogDataQueryParameter = {
            ...
            startIndex: '2026-02-01T04:07:51.000z',
            endIndex: '2026-04-05T14:12:41.000z',
            isGrowing: false,
            mnemonicList: '',
            //  wellUid: this.well,
            //  logUid: 'Surface_Time_RS_1',
            //  wellboreUid: this.wellbore,
            //  logName: 'Surface_Time_RS_1',
            //  indexType: 'date time',
            //  indexCurve: 'RIGTIME',
            //  startIndex: '2026-02-01T04:07:51.000z',
            //  endIndex: '2026-04-05T14:12:41.000z',
        ...
        const timestamp = '2026-02-01T04:07:51.000Z';
        const milliseconds = new Date(timestamp).getTime();

        console.log(milliseconds);
        const endInd = '2026-04-05T14:12:41.000Z';
        const millisecondsEnd = new Date(endInd).getTime();

        console.log(millisecondsEnd);

        this.service
            .getLogDataTime(queryParameter)
            .subscribe({
                next: (logDataArray: any) => {
                    ...
                    if (
                        ...
                        this.parseAndAppendData(logDataArray.logs[0]);
                ...
                error: (err) => {
                    console.error(
                        `❌ [RemoteDS] Fetch failed for ${this.mnemonic}:`,
                        err
                    );
                    this.inFlightRanges.delete(key);
                    if (callback) callback();
    ...
    public parseAndAppendData(reaponse: any): void {
        const mnemonics = reaponse.logData.mnemonicList
            .split(',')
            .map((m: string) => m.trim());
        // In Time logs, the index mnemonic is often 'TIME' instead of 'DEPTH'
        // const timeIdx =
        //   mnemonics.indexOf('RIGTIME') !== -1
        //     ? mnemonics.indexOf('RIGTIME')
        //     : mnemonics.indexOf('DATE');
        ...
        // if (timeIdx === -1 || curveIdx === -1) {
        //   console.warn(
        //     `⚠️ Mnemonic not found. TIME: ${timeIdx}, ${
        //       this.mnemonic
        //     }: ${curveIdx}. Available: ${mnemonics.join(',')}`
        //   );
        //   return;
        // }
        ...
        const currentViewMid = viewport
            ? (viewport.getLow() + viewport.getHigh()) / 2
            : null;

        const newData = reaponse.logData.data
            .map((row: any) => {
                const cols = row.split(',');
                const timeVal = cols[timeIdx]?.trim();
                if (!timeVal) return null;

                // Convert ISO string or timestamp to milliseconds for GeoToolkit
                const d =
                ...
                return isNaN(d) ? null : { d, v };
            })
            .filter((entry: any): entry is { d: number; v: any } => entry !== null);
        ...
        if (newData.length > 0) {
            ...
            // Windowed Virtualization: Prune points outside a sliding window around the viewport
            ...
            if (viewport) {
                ...
                if (pruned.length < unique.length) {
                    console.log(
                        `✂️ [RemoteDS] Virtualization: Pruned ${unique.length - pruned.length
                        } points outside sliding window for ${this.mnemonic}`
                    );
                }
            }

            console.log(
                `📊 [RemoteDS] Merging data for ${this.mnemonic}: current=${current.length}, new=${newData.length}, unique=${unique.length}, final=${pruned.length}`
            );
            this.setData({
...
import { ILogDataQueryParameter, IWellboreObject } from '../../../models/wellbore/wellbore-object';
...
export class DynamicTrackTimeGeneratorComponent implements OnInit, AfterViewInit, OnDestroy {
    ...
    /** Currently selected time scale (default 4 hours) */
    ...
    private registerLithologyPatterns(): void {
        console.log(
            '🧱 Registering lithology patterns from assets/data/lithologyPatterns.json...'
        );
    ...
    //     .getTimeLogHeaders(this.well, this.wellbore)
    //     .subscribe({
    //       next: (headers) => {
    ...
    private loadMudLogData(curve: TrackCurve): Observable<any[]> {
        ...
        return this.http
            ...
            .pipe(
                map((mudLogData) => {
                    console.log(
                        `✅ MudLog data loaded for ${curve.displayName}:`,
                        mudLogData.length,
                        'entries'
                    );
                    ...
                    if (entry && entry.logCurve instanceof StackedLogFill) {
                        console.log(
                            `🔄 Reactively updating MudLog visual for ${curve.displayName}`
                        );
                        this.wellLogWidget?.updateLayout();
                    }
                    return mudLogData;
    ...
    private async createSceneWithData(): Promise<void> {
        ...
        try {
            ...
            indexTrack.setWidth(100); // More width needed for ISO time strings
            indexTrack.setName('Time');

            // Set date time label provider for ISO strings as requested
            ...
            try {
                ...
                console.log(
                    `📏 Setting initial viewport: ${new Date(
                        recentStart
                    ).toISOString()} to ${new Date(scrollTarget).toISOString()}`
                );
                // this.wellLogWidget.setVisibleDepthLimits(recentStart, scrollTarget);
                this.wellLogWidget.setVisibleDepthLimits(1769918871000, 1775398361000);
                ...
                this.wellLogWidget.on(
                    ...
                    (_evt: any, _sender: any, args: any) => {
                        const newLimits = args?.new;
                        if (newLimits) {
                            console.log(
                                `📜 [Scroll] Visible range changed: ${new Date(
                                    newLimits.getLow()
                        ...
                        this.ngZone.run(() => {
                            ...
                            console.log(
                                '📜 VisibleDepthLimitsChanged - triggering requestData on all curves'
                            );
                            ...
                            this.curveMap.forEach((entry) => {
                                ...
                                if (
                                    dataSource &&
                                    dataSource instanceof RemoteLogCurveDataSource
                                ) {
    ...
    private simulateProcessData(
        logId: string,
        data: Record<string, { depths: number[]; values: any[] }>
    ): void {
        ...
        this.curveMap.forEach((entry, mnemonicId) => {
            ...
            if (hasDataSource) {
                if (!logIdGroups.has(entry.info.LogId)) {
                    const header = this.cachedHeaders.find(
                        (h) => h.objectId.includes(entry.info.LogId)
                    );
                    if (header) {
                                ...
                                setTimeout(() => {
                                    ...
                                    if (currentLimits) {
                                        ...
                                        const isUserBrowsingHistory =
                                            fullMaxDepth - currentLimits.getHigh() > currentRange * 5;
        ...
        trackInfo.curves.forEach((curveInfo, curveIndex) => {
            try {
                ...
                if (!curveInfo.data || curveInfo.data.length === 0) {
                    console.log(
                        `ℹ️ Creating empty curve header for ${curveInfo.mnemonicId} (no data)`
                    );
                }

                console.log(`📈 Creating curve: ${curveInfo.mnemonicId}`);
                console.log('this.cachedHeaders ', this.cachedHeaders)
                console.log('curveInfo.LogId ', curveInfo.LogId)
                const header = this.cachedHeaders.find(
                    (h) => h.objectId.includes(curveInfo.LogId)
                );
                if (!header) {
    ...
    private createMudLogCurves(track: LogTrack, trackInfo: ITracks): void {
        ...
        trackInfo.curves.forEach((curveInfo, curveIndex) => {
            try {
                ...
                if (!curveInfo.data || curveInfo.data.length === 0) {
                    console.log(
                        `ℹ️ Creating empty MudLog header for ${curveInfo.displayName} (no data)`
                    );
                }
                ...
                if (mudLogData.depths.length === 0) {
                    console.warn(
                        `⚠️ No valid MudLog data parsed for ${curveInfo.displayName}`
                    );
                    return;
                }
                ...
                console.log(
                    `✅ MudLog curve ${curveInfo.displayName} created successfully with ${mudLogData.depths.length} points`
                );
            } catch (error) {
                console.error(
    ...
    private parseMudLogData(curveInfo: TrackCurve): {
        ...
        try {
            ...
            console.log(
                `📊 Parsed MudLog data: ${depths.length} points, depth range: ${depths.length > 0 ? Math.min(...depths) : 0
                ...
                this.loadLog2DData(curveInfo)
                    .then((log2DData) => {
                        if (!log2DData || log2DData.getRows().length === 0) {
                            console.warn(
                                `⚠️ No valid Log2D data loaded for ${curveInfo.displayName}`
                            );
                            return;
                        }
                        ...
                        console.log(
                            `✅ Log2D curve ${curveInfo.displayName
                            } created successfully with ${log2DData.getRows().length} rows`
                        );
                    })
                    .catch((error) => {
                        console.error(
    ...
    private loadLog2DData(curveInfo: TrackCurve): Promise<Log2DVisualData> {
        ...
        return this.http
            ...
            .then((response) => {
                ...
                console.log(
                    `✅ Loaded Log2D image data: 1:1 mapping, ${log2dData.getRows().length
                    } rows from depth ${log2dData.getMinDepth()} to ${log2dData.getMaxDepth()}`
                );
    ...
    simulateLivePoint(): void {
        ...
        this.curveMap.forEach((entry) => {
            ...
            if (dataSource instanceof RemoteLogCurveDataSource) {
                const depths = dataSource.getDepths();
                if (depths.length > 0)
                    maxLoadedDepth = Math.max(maxLoadedDepth, depths[depths.length - 1]);
            }
        });

        const nextTime =
            maxLoadedDepth > 0 ? maxLoadedDepth + 1000 : new Date().getTime(); // +1 second
        ...
        this.curveMap.forEach((entry) => {
            ...
            if (ds instanceof RemoteLogCurveDataSource) {
                ...
                ds.pushSimulationData([nextTime], [val]);
            }
        });
        ...
        setTimeout(() => {
            ...
            if (limits) {
                ...
                this.wellLogWidget.setVisibleDepthLimits(nextTime - range, nextTime);
                this.wellLogWidget.updateLayout();
            }
        }, 50);
    }
